...
def ZwrocDroge(): ##Wywolywana w glownym programie, by moc potem wypisac wynik w MessageBox
    Droga = float((Predkosc*Czas_Reakcji) + ((Przyspieszenie*Czas**2)/2))
    return Droga
...
def wykres3(CzyPrzykladowe):
    ...
    for i in pylab.frange(0.00, 60, 0.01):#3ci wykres
        ...
        if (CzyPrzykladowe == 0):
            ...
            Czazz3=i/(0.3 * g)
            Czazz4=i/(0.05 *g)
        Drogaaa=float((Czazz * i)-(Tarcie * g * Czazz ** 2) / 2)
        ss.append(Drogaaa)
        if (CzyPrzykladowe == 0):
            Drogaaa=float((Czazz1 * i)-(0.9*g* Czazz1**2)/2)
            ss1.append(Drogaaa)
            Drogaaa=float((Czazz2 * i)-(0.6*g* Czazz2**2)/2)
            ss2.append(Drogaaa)
            Drogaaa=float((Czazz3 * i)-(0.3*g* Czazz3**2)/2)
            ss3.append(Drogaaa)
            Drogaaa=float((Czazz4 * i)-(0.05*g* Czazz4**2)/2)
            ss4.append(Drogaaa)
    pylab.figure()
    pylab.plot(vv, ss)
