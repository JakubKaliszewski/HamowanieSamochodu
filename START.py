...
def statusWlasny(): #zmiana z wybierania z listy na wpisywanie własnego współczynnika
    global status
    status=1
    Dane_wybor.pack_forget()#wylaczenie widocznosci widgetu
...
def helloCallBack():
    ...
    if (status == 0): #jeśli status = 0 to znaczy, że współczynnik został wybrany z listy
        ...
        if(zamiana(E2.get())!=-1 and zamiana(E2.get())<=1): b=zamiana(E2.get())
        else: 
        	error()
        	return
    if(zamiana(E1.get())==-1 or zamiana(E3.get())==-1): error()
        ...
        lines=[''.join([u'Droga całkowita wynosi: \t', string1, '[m]']), ''.join([u'Czas końcowy wynosi: \t', string2, '[s]'])]
        Wyniki='\n'.join(lines)
...
frame1.pack(padx=10)
Tytul=Label(frame1,bg="#4C66FF",fg="#FFFFFF",width=30,text=u"Dane wejściowe.")
Tytul.pack(side=TOP)

Dane=Label(frame1,width=30,bg="#4C66FF",fg="#FFFFFF",justify=LEFT,compound = LEFT, text=u"Podaj predkość początkową[m/s]:")
Dane.pack(side=LEFT)
...
Dane_wlasne=Label(frame2,width=30,bg="#4C66FF",fg="#FFFFFF",justify=LEFT,compound = LEFT,text=u"Podaj współczynnik tarcia:")
Dane_wybor=Label(frame2,width=30,bg="#4C66FF",fg="#FFFFFF",justify=LEFT,compound = LEFT,text=u"Wybierz współczynnik tarcia:")
...
Lista.configure(bg="#4E557F", fg="#FFFFFF")
...
Button_wlasne = Tkinter.Button(frame3,bg="#4E557F",fg="#FFFFFF", text = u"lub podaj własny", command = statusWlasny)
Button_wybor = Tkinter.Button(frame3,bg="#4E557F",fg="#FFFFFF", text = u"lub wybierz z listy", command = statusWybor)
...
Dane=Label(frame4,width=30,bg="#4C66FF",fg="#FFFFFF",justify=LEFT,compound = LEFT,text=u"Podaj czas reakcji kierowcy[s]:")
Dane.pack(side=LEFT)
...
Tkinter.Button(frame5, width=40,bg="#4E557F",fg="#FFFFFF", text=u"Wyświetl wykres s(t)",command = wyswietlWykres1).pack(side=LEFT)
Checkbutton(frame5,bg="#999999", text=u"+ wykresy przykładowych współczynników", variable = przykladowe1, onvalue=0, offvalue=1).pack(side=RIGHT)
frame6=Frame(window, bg="#4C66FF")
frame6.pack(side=TOP, pady=5)
Tkinter.Button(frame6, width=40,bg="#4E557F",fg="#FFFFFF", text=u"Wyświetl wykres v(t)", command = wyswietlWykres2).pack(side=LEFT)
Checkbutton(frame6,bg="#999999", text=u"+ wykresy przykładowych współczynników", variable = przykladowe2, onvalue=0, offvalue=1).pack(side=RIGHT)
frame7=Frame(window, bg="#4C66FF")
frame7.pack(side=TOP, pady=5)
Tkinter.Button(frame7, width=40,bg="#4E557F",fg="#FFFFFF", text=u"Wyświetl wykres drogi od prędkości początkowej", command = wyswietlWykres3).pack(side=LEFT)
Checkbutton(frame7,bg="#999999", text=u"+ wykresy przykładowych współczynników", variable = przykladowe3, onvalue=0, offvalue=1).pack(side=RIGHT)
#========================================================================================================
# obliczanie
#========================================================================================================
Tkinter.Button(window, width=60,bg="#4E557F",fg="#FFFFFF", text=u"Oblicz drogę hamowania i zapisz wynik do pliku", command = helloCallBack).pack(pady=5)
